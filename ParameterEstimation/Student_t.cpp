...
ublas::vector<double> Student_t::calc_gradients(ublas::vector<double> const& x) {
    update_garch_vec(x);
    ...
    gradients(2) = db;
    gradients(3) = dmu;
    gradients(4) = calc_num_gradients(x)(4);

    return gradients;
}

ublas::vector<double> Student_t::calc_num_gradients(ublas::vector<double> const& x) {
...
double Student_t::calc_step_size(
    ...
    while (function_value(x + a * d) > function_value(x) + c1 * a * inner_prod(calc_gradients(x), d)) {
        a *= 0.5;
