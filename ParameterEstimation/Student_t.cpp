...
double Student_t::function_value(vector<double> x) {
	...
	for (size_t i = 1; i < m_GARCH_vec.size() - 1; ++i) {
		sum += log(m_GARCH_vec(i))
			- (x(4) + 1) / 2 * log(1 + pow(time_series(i + 1), 2) / (m_GARCH_vec(i) * (x(4) - 2)));
	} 
...
vector<double> Student_t::derivative_w(vector<double> x) {
...
vector<double> Student_t::derivative_a(vector<double> x) {
...
vector<double> Student_t::derivative_b(vector<double> x) {
...
vector<double> Student_t::calcGradients(vector<double> x) {
	...
	for (size_t i = 0; i < m_GARCH_vec.size() - 1; i++) {
		...
		temp = 0.5 * (m_GARCH_vec(i) - pow(time_series(i) - x(3) / 252, 2) * 252) / (pow(m_GARCH_vec(i), 2));

		dmu = dmu - 2 * (time_series(i) - x(3) / 252) / m_GARCH_vec(i);
