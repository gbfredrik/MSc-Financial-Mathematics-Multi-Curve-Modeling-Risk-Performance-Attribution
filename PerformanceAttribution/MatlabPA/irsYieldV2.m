...
for i = 1:numCashFix
    ...
    fix = fix + deltaTj(i - 1) * exp(-fixCashFlows(i)/365 * rZero(fixCashFlows(i) + 1));
...
for i = 1:numCashFloat - 1
    float = float + exp(floatCashFlows(i + 1)/365 * rTau(floatCashFlows(i + 1) + 1) - floatCashFlows(i)/365 * rTau(floatCashFlows(i) + 1) ...
        - floatCashFlows(i + 1)/365 * rZero(floatCashFlows(i + 1) + 1)) ...
        - exp(-floatCashFlows(i + 1)/365 * rZero(floatCashFlows(i + 1) + 1));
end
