...
void placeholder_eigen(
    ...
    if (save) {
        write_csv_matrix(curve_collection.m_E, curve_collection.file_path + name_prefix +
            "_eigvec_" + curve_collection.file_name);
        write_csv_vector(curve_collection.v_Lambda, curve_collection.file_path + name_prefix +
            "_eigval_" + curve_collection.file_name);
    }
}
