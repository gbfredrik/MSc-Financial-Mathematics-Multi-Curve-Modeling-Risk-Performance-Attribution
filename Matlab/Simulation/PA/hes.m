function [H] = hes(r, pi, fixDatesIndexes, floatDatesIndexes, timeFracFix, timeFracFloat, dtFix, dtFloat, fixing, RoP, y, N, numKnown, aZero, aPi)
    ...
    for i = 1:numKnown
        float = float + fixing(i) * dtFloat(i) * N * (-timeFracFloatKnown(i+1) * aZero(floatDatesKnownIndexes(i+1),:))'...
            * (-timeFracFloatKnown(i+1) * aZero(floatDatesKnownIndexes(i+1),:)) ...
    ...
    for i = numKnown+1:numFloat-1
        ...
        * (timeFracFloat(i + 1) * aPi(floatDatesIndexes(i + 1),:) - timeFracFloat(i) * (aZero(floatDatesIndexes(i),:) + aPi(floatDatesIndexes(i),:))) ...
        * exp(timeFracFloat(i+1) * pi(floatDatesIndexes(i + 1)) - timeFracFloat(i) * (r(floatDatesIndexes(i)) + pi(floatDatesIndexes(i))))'...
        - (-timeFracFloat(i+1) * aZero(floatDatesIndexes(i+1),:))' * (-timeFracFloat(i+1) * aZero(floatDatesIndexes(i+1),:)) ...
